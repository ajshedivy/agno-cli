...
export const sessionCommand = new Command("session").description("Manage sessions");
...
sessionCommand
	...
	.action(async (_options, cmd) => {
		try {
			const opts = cmd.optsWithGlobals();
			const sessionIds = (opts.ids as string).split(",").map((s: string) => s.trim());
			const sessionTypes = (opts.types as string).split(",").map((s: string) => s.trim());
