...
export const traceCommand = new Command("trace").description("Manage traces");
...
traceCommand
	...
	.action(async (_options, cmd) => {
		try {
			...
			// Attempt structured table rendering if result has expected shape
			...
			if (Array.isArray(r.data)) {
				...
				// Fallback: print raw JSON when shape is unexpected
				printJson(result);
			}
		} catch (err) {
			handleError(err, { url: getBaseUrl(cmd) });
...
traceCommand
	...
	.action(async (_options, cmd) => {
		try {
			...
			// Best-effort table rendering of search results
			const r = result as Record<string, unknown>;
			if (Array.isArray(r.data)) {
				const data = r.data as Record<string, unknown>[];
				outputList(
