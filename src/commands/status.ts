...
import { getOutputFormat, outputDetail, outputList, printJson } from "../lib/output.js";
...
export const statusCommand = new Command("status")
	...
	.action(async (_options, cmd) => {
		try {
			...
			const raw = (await client.getConfig()) as unknown as Record<string, unknown>;
...
function renderOsIdentity(cmd: Command, config: ConfigShape): void {
	const display: Record<string, unknown> = { os_id: config.os_id ?? "unknown" };
...
function renderDatabases(config: ConfigShape): void {
	...
	for (const db of dbs) {
		const marker = db === config.os_database ? chalk.dim(" (primary)") : "";
...
function renderAgents(cmd: Command, agents: ResourceSummary[] | undefined): void {
...
function renderWorkflows(cmd: Command, workflows: ResourceSummary[] | undefined): void {
...
function renderKnowledge(cmd: Command, instances: KnowledgeInstance[] | undefined): void {
...
function renderInterfaces(cmd: Command, interfaces: InterfaceSummary[] | undefined): void {
