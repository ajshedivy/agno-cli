...
export const scheduleCommand = new Command("schedule").description("Manage schedules");
...
scheduleCommand
	...
	.requiredOption("--method <method>", "HTTP method: GET, POST, etc. (required)")
	...
	.option("--timeout-seconds <n>", "Timeout in seconds", (v: string) => Number.parseInt(v, 10))
	.option("--max-retries <n>", "Maximum retries", (v: string) => Number.parseInt(v, 10))
	.option("--retry-delay-seconds <n>", "Retry delay in seconds", (v: string) => Number.parseInt(v, 10))
	.action(async (_options, cmd) => {
		try {
			const opts = cmd.optsWithGlobals();
			const client = getClient(cmd);
